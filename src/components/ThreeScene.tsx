...
/**
 * When the render loop is set to "demand" the canvas only draws new frames when
 * something asks for them. This tiny helper invalidates the canvas whenever a
 * customization prop changes so the new material/color/wireframe state is still
 * painted even without a running loop.
 */
...
function ThreeScene({ color, material, wireframe, autoRotate }: ThreeSceneProps) {
  ...
  return (
    <Suspense fallback={<LoadingFallback />}>
      <div
        role="img"
        aria-label="3D view of the fashion tote. Drag to rotate and pinch or scroll to zoom."
      ...
      >
        <Canvas
          ...
          frameloop={reducedMotion || !autoRotate ? "demand" : "always"}
        >
           ...
           * once (`frames={1}`), keeping the perf cost negligible. 128px is
           * enough for the soft reflections on a small product canvas and
           * cuts the one-off offscreen render cost several times over.
           */}
